...
# Completion provider: conflicting file paths (when a rebase/merge/cherry-pick is paused).
#
# Expected args:
# - .repoPath (optional): absolute path to target git repository
# - .query or .prefix (optional): partial path
...
conflicts_raw="$(git -C "${repo_path}" diff --name-only --diff-filter=U 2>/dev/null || true)"
