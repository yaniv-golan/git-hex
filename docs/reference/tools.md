...
- **git-hex-continueOperation** — Runs `rebase --continue`, `cherry-pick --continue`, or `merge --continue`, returning `completed`/`paused` with conflicting files when paused. Returns an error for detected but unsupported operations (revert, bisect).
- **git-hex-abortOperation** — Aborts the in-progress rebase/merge/cherry-pick and restores the original state. Returns an error for detected but unsupported operations (revert, bisect).
