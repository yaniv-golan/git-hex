...
FRAMEWORK_VERSION_DEFAULT="v0.8.3"
# Pinned commit for v0.8.3 installs (annotated tags have distinct object SHAs).
FRAMEWORK_GIT_SHA_DEFAULT_V083="d9e2e08e516e6c75e89effdbae4da572cd3986f5"
...
if [ "${GIT_HEX_DELEGATE_DOCTOR}" != "true" ]; then
	if [ "${GIT_HEX_DOCTOR_MODE}" = "true" ] && [ "${GIT_HEX_DOCTOR_FIX_MODE}" = "true" ] && [ -n "${MCPBASH_HOME:-}" ]; then
		...
		die 3 "ERROR: install does not modify MCPBASH_HOME-managed installs (${FRAMEWORK_DIR}). Unset MCPBASH_HOME to use the managed default (${DEFAULT_FRAMEWORK_DIR}), or upgrade that install manually."
	fi

	if [ "${GIT_HEX_DOCTOR_MODE}" = "true" ] && [ "${GIT_HEX_DOCTOR_FIX_MODE}" = "true" ] && [ "${FRAMEWORK_DIR}" != "${DEFAULT_FRAMEWORK_DIR}" ]; then
		die 3 "ERROR: install only operates on the managed default install path (${DEFAULT_FRAMEWORK_DIR}); current framework path is ${FRAMEWORK_DIR}."
	fi
fi
...
atomic_replace_dir() {
	...
	die 2 "Failed to replace ${target_dir} with staged install; re-run install to retry."
}
...
if [ "${framework_exists}" != "true" ] || [ "${framework_too_old}" = "true" ]; then
	...
	if [ -n "${GIT_HEX_MCPBASH_SHA256:-}" ]; then
		...
		if [ -z "${expected_git_sha}" ] && [ "${FRAMEWORK_VERSION}" = "${FRAMEWORK_VERSION_DEFAULT}" ]; then
			expected_git_sha="${FRAMEWORK_GIT_SHA_DEFAULT_V083}"
		fi
...
# mcp-bash 0.8.3+ includes icons format validation and --inspector flag
if [ "${1:-}" = "validate" ]; then
