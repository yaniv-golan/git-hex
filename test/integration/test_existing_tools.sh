...
if run_tool_expect_fail gitHex.createFixup "${REPO_FIX_FAIL}" '{"commit": "HEAD"}'; then
...
result_fix="$(run_tool gitHex.createFixup "${REPO_FIX_OK}" "{\"commit\": \"${target_commit}\"}")"
...
if run_tool_expect_fail gitHex.amendLastCommit "${REPO_AMD_FAIL}" '{}'; then
...
result_amend_stage="$(run_tool gitHex.amendLastCommit "${REPO_AMD_STAGE}" '{}')"
...
result_amend_msg="$(run_tool gitHex.amendLastCommit "${REPO_AMD_MSG}" '{"message": "New message"}')"
...
result_amend_both="$(run_tool gitHex.amendLastCommit "${REPO_AMD_BOTH}" '{"message": "Both"}')"
