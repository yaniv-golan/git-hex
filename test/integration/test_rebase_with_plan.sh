...
run_tool git-hex-rebaseWithPlan "${REPO_NO_AUTO}" '{"onto": "HEAD~3", "autoStash": false, "autosquash": false, "plan": []}' 60 >/dev/null
...
run_tool git-hex-rebaseWithPlan "${REPO_SQUASH_MSG}" "$(jq -n --argjson p "${plan_squash}" '{onto:"HEAD~2", plan:$p, requireComplete:true}')" 90 >/dev/null
