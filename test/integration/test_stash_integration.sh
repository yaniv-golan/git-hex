...
if run_tool_expect_fail git-hex-rebaseWithPlan "${REPO3}" '{"onto": "main", "autoStash": false}' 60; then
...
if run_tool_expect_fail git-hex-rebaseWithPlan "${REPO4}" '{"onto": "main"}' 60; then
...
if run_tool_expect_fail git-hex-amendLastCommit "${REPO8}" '{"autoStash": true, "message": "Should fail"}'; then
...
result="$(run_tool git-hex-amendLastCommit "${REPO9}" '{"autoStash": true, "message": "Trigger stash pop conflict"}')"
