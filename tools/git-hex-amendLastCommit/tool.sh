...
if ! git -C "${repo_path}" rev-parse HEAD >/dev/null 2>&1; then
	git_hex_error "NO_COMMITS" "Repository has no commits to amend." \
		"Create at least one commit before using amendLastCommit"
fi
...
if [ -z "${staged_files}" ] && [ -z "${new_message}" ]; then
	git_hex_error "NO_STAGED_CHANGES" "Nothing to amend." \
		"Stage changes with 'git add' before amending" \
		"Or provide a new message to update the commit message only"
fi
...
if ! commit_error="$(git -C "${repo_path}" commit "${amend_args[@]}" 2>&1)"; then # bash32-safe: amend_args always contains --amend (line 67)
	...
	git_hex_fail_commit_error "Failed to amend commit" "${commit_error}"
fi
