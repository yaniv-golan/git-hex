...
if  [ -z "${operation}" ]; then
	git_hex_error "INVALID_ARGUMENT" "No rebase, merge, or cherry-pick in progress." \
		"Use getConflictStatus to check the current repository state" \
		"Start a rebase with rebaseWithPlan before using continueOperation"
fi
