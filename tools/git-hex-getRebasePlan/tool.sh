...
if [ -z "${empty_tree_sha}" ]; then
	git_hex_error "GIT_ERROR" "Failed to compute empty tree hash" \
		"Ensure git is working correctly in this repository"
fi
	...
	if [ -z "${onto}" ]; then
		...
		if [ "${total_commits}" -eq 0 ]; then
			git_hex_error "NO_COMMITS" "Repository has no commits" \
				"Create at least one commit before using getRebasePlan"
		fi
