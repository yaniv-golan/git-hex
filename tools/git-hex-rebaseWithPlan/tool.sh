...
if  [ "${plan_type}" != "array" ]; then
	git_hex_error "INVALID_ARGUMENT" "plan must be an array." \
		"Use getRebasePlan to generate a valid plan" \
		"Then modify and pass it to rebaseWithPlan"
fi
...
if ! git -C "${repo_path}" rev-parse HEAD >/dev/null 2>&1; then
	git_hex_error "NO_COMMITS" "Repository has no commits. Nothing to rebase." \
		"Create at least one commit before using rebaseWithPlan"
fi
...
if  ! git -C "${repo_path}" rev-parse --verify "${onto}^{commit}" >/dev/null 2>&1; then
	if git_hex_is_shallow_repo "${repo_path}"; then
		git_hex_error "INVALID_COMMIT" "Invalid onto ref: ${onto} (repository is shallow - try 'git fetch --unshallow')" \
			"Run 'git fetch --unshallow' to fetch full history" \
			"Or use a commit that exists in the shallow clone"
	fi
	git_hex_error "INVALID_COMMIT" "Invalid onto ref: ${onto}" \
		"Use a valid branch name, tag, or commit SHA" \
...
validate_rebase_action()  {
	...
	pick | reword | squash | fixup | drop)
		...
		;;
	*)
		git_hex_error "INVALID_ACTION" "Invalid action '${action}'." \
			"Must be one of: pick, reword, squash, fixup, drop"
		;;
	esac
}
...
if  [ "${plan_length}" -gt 0 ]; then
	...
	if [ "${has_null_message}" = "true" ]; then
		git_hex_error "INVALID_MESSAGE" "Commit message cannot contain null bytes" \
			"Use plain text without null characters"
	fi
fi
...
if   [ "${plan_length}" -gt 0 ]; then
	...
	while IFS= read -r -d '' action && IFS= read -r -d '' commit_ref && IFS= read -r -d '' message; do
		...
		if [ -z "${full_hash}" ]; then
			git_hex_error "INVALID_COMMIT" "Invalid commit reference: ${commit_ref}" \
				"Use a valid commit SHA or reference from getRebasePlan"
		fi
		if ! grep -Fxq -- "${full_hash}" <<<"${actual_commits}"; then
			git_hex_error "COMMIT_NOT_IN_RANGE" "Commit ${commit_ref} (${full_hash:0:7}) not in rebase range ${onto}..HEAD" \
				"Use getRebasePlan with the same 'onto' value to see commits in range"
		fi
		...
		for existing in ${plan_commits[@]+"${plan_commits[@]}"}; do
			if [ "${existing}" = "${full_hash}" ]; then
				git_hex_error "DUPLICATE_COMMIT" "Duplicate commit in plan: ${commit_ref}" \
					"Each commit can only appear once in the plan"
			fi
		done
		if [ -n "${message}" ] && { [[ "${message}" == *$'\t'* ]] || [[ "${message}" == *$'\n'* ]]; }; then
			git_hex_error "INVALID_MESSAGE" "Commit message cannot contain TAB or newline characters" \
				"Use plain text without tabs or newlines for the message field"
		fi
		if [ "${action}" = "reword" ] && [ -z "${message}" ]; then
			git_hex_error "MISSING_MESSAGE" "Action 'reword' requires a 'message' field" \
				"Provide a 'message' field with the new commit message" \
				"Or use 'pick' to keep the original message"
		fi
		plan_actions+=("${action}")
...
if [ "${require_complete}" = "true" ]; then
	if [ "${plan_length}" -ne "${actual_count}" ]; then
		git_hex_error "INCOMPLETE_PLAN" "Plan has ${plan_length} commits but range has ${actual_count}" \
			"Include all commits with requireComplete=true" \
			"Use 'drop' action to exclude commits" \
			"Or set requireComplete=false"
	fi
fi
...
if [ "${rebase_status}" -eq 0 ]; then
	...
	if [ "${has_unmerged}" = "true" ] || { [ "${rebase_in_progress}" = "true" ] && grep -qE '(^|\n)CONFLICT' <<<"${rebase_output}"; }; then
		...
		if [ "${abort_on_conflict}" = "false" ]; then
			...
			_git_hex_cleanup_rebase_msg_dir "${rebase_msg_dir_marker}"
			git_hex_error "REBASE_CONFLICT" "Rebase failed due to conflicts. Repository restored to original state." \
		...
		git -C "${repo_path}" rebase --abort >/dev/null 2>&1 || true
		_git_hex_cleanup_rebase_msg_dir "${rebase_msg_dir_marker}"
		git_hex_error "NOTHING_TO_REBASE" "Nothing to rebase." \
			"HEAD is already at the onto target, or all commits are already applied"
	else
		git -C "${repo_path}" rebase --abort >/dev/null 2>&1 || true
		_git_hex_cleanup_rebase_msg_dir "${rebase_msg_dir_marker}"
		error_hint="${rebase_output%%$'\n'*}"
		git_hex_error "REBASE_FAILED" "Rebase failed: ${error_hint}" \
			"Use undoLast to restore if needed" \
