...
if [ "${splits_type}" != "array" ]; then
	git_hex_error "INVALID_ARGUMENT" "splits must be an array" \
		"Provide an array of split objects with files and message properties"
fi
...
case "${operation}" in
rebase) git_hex_error "REBASE_IN_PROGRESS" "Cannot split commit while rebase is in progress" \
	"Complete or abort the current rebase first" ;;
cherry-pick) git_hex_error "CHERRY_PICK_IN_PROGRESS" "Cannot split commit while cherry-pick is in progress" \
	"Complete or abort the current cherry-pick first" ;;
revert) git_hex_error "REVERT_IN_PROGRESS" "Cannot split commit while revert is in progress" \
	"Complete or abort the current revert first" ;;
merge) git_hex_error "MERGE_IN_PROGRESS" "Cannot split commit while merge is in progress" \
	"Complete or abort the current merge first" ;;
bisect) git_hex_error "BISECT_IN_PROGRESS" "Cannot split commit while bisect is in progress" \
	"Complete or abort the current bisect first" ;;
esac

# Resolve commit
full_commit="$(git -C "${repo_path}" rev-parse --verify "${commit_ref}^{commit}" 2>/dev/null || true)"
if [ -z "${full_commit}" ]; then
	git_hex_error "INVALID_COMMIT" "Invalid commit: ${commit_ref}" \
...
if git -C "${repo_path}" rev-parse --verify "${full_commit}^2" >/dev/null 2>&1; then
	git_hex_error "MERGE_COMMIT" "Cannot split merge commits" \
		"Merge commits cannot be split" \
		"Split the individual commits from the merged branch instead"
fi

# Commit must not be root commit
if ! git -C "${repo_path}" rev-parse "${full_commit}^" >/dev/null 2>&1; then
	git_hex_error "ROOT_COMMIT" "Cannot split root commit" \
		"The root commit (first commit) cannot be split"
fi

# Commit must be ancestor of HEAD
if ! git -C "${repo_path}" merge-base --is-ancestor "${full_commit}" HEAD 2>/dev/null; then
	git_hex_error "NOT_ANCESTOR" "Commit ${commit_ref} is not an ancestor of HEAD; cannot split from this branch" \
		"Switch to a branch that contains this commit" \
		"Or specify a commit that is in your current branch's history"
fi
...
if [ "${split_count}" -lt 2 ]; then
	git_hex_error "INVALID_ARGUMENT" "At least 2 splits required" \
		"Provide at least 2 split objects to split a commit"
fi

original_files="$(git -C "${repo_path}" diff-tree --no-commit-id --name-only -r "${full_commit}" --)"
if [ -z "${original_files}" ]; then
	git_hex_error "EMPTY_COMMIT" "Commit ${commit_ref} has no file changes" \
		"Cannot split a commit with no file changes"
fi
...
for ((i = 0; i < split_count; i++)); do
	...
	case "${message}" in
	*$'\n'* | *$'\t'*)
		git_hex_error "INVALID_ARGUMENT" "Split message cannot contain TAB or newline" \
	...
	if [ "${split_files_len}" -eq 0 ]; then
		git_hex_error "INVALID_ARGUMENT" "Split $((i + 1)) has no files" \
			"Each split must include at least one file"
	fi
	...
	for ((j = 0; j < file_count; j++)); do
		...
		if ! array_contains "${file}" "${original_files_arr[@]}"; then
			git_hex_error "FILE_NOT_IN_COMMIT" "File '${file}' not in original commit" \
				"Only files that were changed in the original commit can be split"
		fi
		...
		if [ "${#covered_files_arr[@]}" -gt 0 ] && array_contains "${file}" "${covered_files_arr[@]}"; then
			git_hex_error "DUPLICATE_FILE" "File '${file}' appears in multiple splits" \
				"Each file can only be assigned to one split"
		fi
...
for file in "${original_files_arr[@]}"; do
	...
	if ! array_contains "${file}" "${covered_files_arr[@]}"; then
		git_hex_error "FILE_NOT_ASSIGNED" "File '${file}' from original commit not assigned to any split" \
			"Every file from the original commit must be assigned to exactly one split"
	fi
done
...
if { [ -z "${rebase_merge_dir}" ] || [ ! -d "${rebase_merge_dir}" ]; } && { [ -z "${rebase_apply_dir}" ] || [ ! -d "${rebase_apply_dir}" ]; }; then
	...
	if [ "${rebase_status}" -ne 0 ]; then
		git_hex_error "REBASE_FAILED" "Rebase failed before split: ${error_hint}" \
			"Check that the commit is in the current branch's history" \
			"Use 'git status' to check repository state"
	else
		git_hex_error "REBASE_FAILED" "${error_hint}" \
			"Check that the commit is in the current branch's history"
	fi
fi
...
for ((i = 0; i < split_count; i++)); do
	...
	if [ "${sign_commits}" = "true" ]; then
		if ! commit_error="$(git -C "${repo_path}" commit -F "${msg_file}" 2>&1)"; then
			...
			git_hex_error "COMMIT_FAILED" "Failed to create split commit $((i + 1))/${split_count}: ${error_hint}" \
				"The repository may be in an inconsistent state" \
				"Use 'git status' to diagnose and 'git rebase --abort' to recover"
		fi
	else
